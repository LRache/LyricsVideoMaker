...
def get_one_lyrics_text_clip(lyrics: str, configures: Configures,
    ...
    clips = []
    if advance:
        pointClip0 = ImageClip("circle.png").set_position(configures.advancePointPositionList[0]) \
            .set_start(max(currentStartTime - 6, 0)).set_end(currentStartTime-0.8)
        pointClip1 = ImageClip("circle.png").set_position(configures.advancePointPositionList[1])\
            .set_start(max(currentStartTime - 6, 0)).set_end(currentStartTime-1.8)
        pointClip2 = ImageClip("circle.png").set_position(configures.advancePointPositionList[2]) \
